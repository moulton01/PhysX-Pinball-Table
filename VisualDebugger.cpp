...
namespace VisualDebugger
{
	...
	void Init(const char *window_name, int width, int height)
	{
		...
		camera = new Camera(PxVec3(-9.0f, 10.0f, 9.0f), PxVec3(0.f,-.1f,-1.f), 5.f);
	...
	{ 
		...
		string playerScore; 
		ostringstream convertPlayerScore;
		convertPlayerScore << score; 
		playerScore = convertPlayerScore.str(); 
		//initialise HUD
		//add an empty screen
		hud.AddLine(EMPTY, "");
		hud.AddLine(EMPTY, "");
		hud.AddLine(EMPTY, "");
		//add a help screen
		hud.AddLine(HELP, "Score: " + playerScore); 
		hud.AddLine(EMPTY, "");
		hud.AddLine(HELP, " R for Left Flipper, T for Right Flipper");
		hud.AddLine(HELP, " K to retract plunger");
	...
	void ForceInput(int key)
	{
		...
		case 'I': //forward
			...
			break;
		case 'K': //backward
			scene->GetSelectedActor()->addForce(PxVec3(0,-1.0,+6)*gForceStrength);
			break;
		case 'J': //left
